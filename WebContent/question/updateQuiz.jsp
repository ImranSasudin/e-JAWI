						...
						<div class="col-md-8">
							<ul class="pagination pg-primary justify-content-center">
								<c:forEach var="q" items="${questions}">
									<li class="page-item ${q.questionNumber == question.questionNumber ? 'active' : ' ' }">
									<a class="page-link" href="/e-JAWI/QuestionController?action=nextQ&quizId=<c:out value="${quiz.quizId }" />&number=<c:out value="${q.questionNumber }" />">
									<c:out value="${q.questionNumber }"/></a></li>
								</c:forEach>
							</ul>
						</div>
					</div>
							...
							<div class="card">
								<div class="card-header">
									<div class="d-flex align-items-center">
										<h4 class="card-title">
											Question Num :
											<c:out value="${question.questionNumber }" />
										</h4>
									</div>
								</div>
								<div class="card-body">
									<form action="/e-JAWI/QuestionController" method="post">
										<input type="hidden" name="questionID" value="<c:out value="${question.questionId }" />"/>
										<input type="hidden" name="questionNumber" value="<c:out value="${question.questionNumber }" />"/>
										<input type="hidden" name="quizId" value="<c:out value="${quiz.quizId }" />"/>
										<div class="row mt-3">
											<div class="col-md-12">
										...
										<div class="text-left mt-3 mb-3">
											<button class="btn btn-primary" name="action" value="UpdateAnswer">Save</button>
										</div>
									</form>
								</div>
							</div>
							<div class="text-left mt-3 mb-3">
								<a class="btn btn-success" href="/e-JAWI/QuizController?action=unansweredQ&quizId=<c:out value="${quiz.quizId }" />">Submit</a>
							</div>
						</div>
					</div>
				</div>
