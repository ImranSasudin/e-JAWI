											...
											<div class="col-md-12">
												<c:choose>
													<c:when test="${not empty questions}">
														<div class="alert alert-danger" role="alert">UNANSWERED QUESTION</div>
														<c:forEach var="q" items="${questions }">
															<a class="text-white btn btn-round btn-danger my-3 mx-2" href="/e-JAWI/QuestionController?action=nextQ&quizId=<c:out value="${quiz.quizId }" />&number=<c:out value="${q.questionNumber }" />">
															<c:out value="${q.questionNumber }"/></a>
														</c:forEach>
													</c:when>
													<c:otherwise>
														<div class="alert alert-success" role="alert">ALL QUESTIONS ARE ANSWERED!</div>
													</c:otherwise>
												</c:choose>
											</div>
										</div>
										<div class="mt-3 mb-3">
											<a class="btn btn-success btn-lg btn-block" href="/e-JAWI/QuizController?action=submit&quizId=<c:out value="${quiz.quizId }" />">Submit</a>
										</div>
									</form>
								</div>
							</div>
